...
class Erp
{
	...
	function convertQty($_item_code = NULL, $_qty = NULL){
		$_select_all_units = $this->site->getUnitUOM($_item_code);
		foreach ($_select_all_units as $_get_unit)
		{
			$_unit_description = $_get_unit->name;
			...
			$_cost	           = ($_get_unit->qty_unit * $_get_unit->pcost );
			$_price			   = $_get_unit->price;
			$_c_qty = $_qty / $_unit_qty;
			
			$_units[] = "<tr><td>$_unit_description</td><td >".$this->formatQuantity($_c_qty)."</td>".($this->Owner || $this->Admin ?"<td>".$this->formatMoney($_cost)."</td><td>".$this->formatMoney($_price)."</td>":"")."</tr>";
		}
		$_string_unit = $this->array_2_string ("", $_units);
		return "$_string_unit";
	}
