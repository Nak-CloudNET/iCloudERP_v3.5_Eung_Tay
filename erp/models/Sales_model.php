...
class Sales_model extends CI_Model
{
	...
	public function getProductNumber($term, $warehouse_id, $standard, $combo, $digital, $service, $category, $limit = 100)
    {
		if(preg_match('/\s/', $term))
		{
			$name = explode(" ", $term);
			$first = $name[0];
			$this->db->select('products.id, code, name, type, cost,warehouses_products.quantity as qoh,warehouses_products.quantity, price, tax_rate, tax_method, cf1, product_details,subcategory_id, details,COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
			...
			$this->db->select('products.id, code, name, type, cost, warehouses_products.quantity as qoh,warehouses_products.quantity, price, tax_rate, tax_method, product_details, details, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) FROM erp_serial as sp WHERE sp.product_id='.$this->db->dbprefix('products').'.id ), "") as sep, cf1')
            ->join('warehouses_products', 'warehouses_products.product_id=products.id', 'left')
            ->group_by('products.id');
			if($this->Owner || $this->admin){
