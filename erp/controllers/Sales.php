...
class Sales extends MY_Controller
{
	...
	function index($warehouse_id = NULL)
    {
        ...
        $arr1 = [];
        $i = 0; $j = 0;
        $sql = "select transaction_id, product_id from erp_purchase_items
        ...
        foreach ($results as $result){//$this->erp->print_arrays($result);
            ...
            if($results1){
                $arr[$i] = array('result' => $result->transaction_id,'product_id' => $result->product_id);
                $i++;
            }else{
                $arr1[$j] = array('result' => $result->transaction_id,'product_id' => $result->product_id);
                $j++;
            }
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
				...
				if ($this->input->post('form_action') == 'PNP_statement' || $this->input->post('form_action') == 'export_pdf') {
					...
					$this->excel->getActiveSheet()->setCellValue('A2','#46A St.606, Group 43, Beoung Kak II, Toul Kork, Phnom Penh.');
					$this->excel->getActiveSheet()->setCellValue('A3','Tel / Fax : 023 640 4828  H/P : 011/ 077/ 0887/ 081 : 63 82 82, 092 838823');
					$this->excel->getActiveSheet()->setCellValue('A4','SOLD TO:');
					$this->excel->getActiveSheet()->setCellValue('A5','COMIN KHMER CO., LTD');
					$this->excel->getActiveSheet()->setCellValue('A7','Contact : 023 885 640-9 H/P : 011 801 063');
					$this->excel->getActiveSheet()->setCellValue('D4','INVOICE NUMBER  :');
					$this->excel->getActiveSheet()->setCellValue('E4','1712-0015');
                    ...
                    if ($this->input->post('form_action') == 'export_pdf') {
						...
						$this->excel->getActiveSheet()->getStyle('A2:E2')->applyFromArray($header_style);
						$this->excel->getActiveSheet()->getStyle('A3:E3')->applyFromArray($header_style);
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($sale_order_id){
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->tax_rate) {
                        $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'expdates'=>$expdates, 'makeup_cost' => 0,'group_prices'=>$group_prices,'customer_percent' => $customer_percent, 'all_group_price' => $all_group_prices);
                    } else {
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'makeup_cost' => 0, 'expdates'=>$expdates, 'customer_percent' => $customer_percent,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices);
                    }
                    $c++;
                }
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products, $sale_data, $payment, (isset($loans)?$loans:""))) {
            ...
            foreach ($inv_items as $item) {
                ...
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'expdates'=>$expdates,'makeup_cost' => 0, 'group_prices' => $group_prices,'customer_percent' => $customer_percent, 'all_group_price' => $all_group_prices,'slaeid'=>$item->id);
                } else {
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'expdates'=>$expdates,'makeup_cost' => 0, 'group_prices' => $group_prices,'customer_percent' => $customer_percent, 'all_group_price' => $all_group_prices,'slaeid'=>$item->id);
                }
                $c++;
            }
            if ($this->session->userdata('biller_id')) {
		...
		if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")" . " (" . $row->price . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'expdates'=>$expdates,'cost' => $row->cost, 'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'orderqty'=>$orderqty, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_percent,'currency'=>$currency,'us_currency'=>$us_currency,'makeup_cost_percent'=>$percent->percent);
                } else {
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")" . " (" . $row->price . ")" , 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options,'expdates'=>$expdates,'cost' => $row->cost,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'orderqty'=>$orderqty, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_percent,'currency'=>$currency,'us_currency'=>$us_currency,'makeup_cost_percent'=>$percent->percent);
                }
            }
			echo json_encode($pr);
        } else {
        ...
        if ($rows) {
            foreach ($rows as $row) {
				...
				if ($row->tax_rate) {
					...
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'expdates'=>$expdates,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_percent, 'makeup_cost_percent'=>$percent->percent);
					
				} else {
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options,$options,'expdates'=>$expdates,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_percent, 'makeup_cost_percent'=>$percent->percent);
				}
		...
		if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'expdates'=>$expdates, 'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_percent, 'makeup_cost_percent'=>$percent->percent);
                }
            }
            echo json_encode($pr);
        } else {
	...
	function invoice_nano_tech()
	{
		$this->load->view($this->theme .'sales/invoice_nano_tech');
	}
}